...
Public Class Affairs
    ...
    Friend Sub Load()
        ...
        'GroupControlMain
        '
        If Me.GroupControl.Tag.ToString <> "Affairs" Then
            Clean_GroupControl()
    ...
    End Sub
    Private Sub Allow()
        If Not (Internet.AffairsResult(Form.CurrentSocket, AID, 1, Form.LoginCode)) Then
            MsgBox("事务处理失败", vbExclamation, "MAC Client")
            Selected()
        End If
    End Sub
    Private Sub Reject()
        If Not (Internet.AffairsResult(Form.CurrentSocket, AID, 2, Form.LoginCode)) Then
            MsgBox("事务处理失败", vbExclamation, "MAC Client")
            Selected()
        End If
    End Sub
    Private Sub AllowAsRule()
        If Not (Internet.AffairsResult(Form.CurrentSocket, AID, 3, Form.LoginCode)) Then
            MsgBox("事务处理失败", vbExclamation, "MAC Client")
            Selected()
        End If
    End Sub
    Private Sub RejectAsRule()
        If Not (Internet.AffairsResult(Form.CurrentSocket, AID, 4, Form.LoginCode)) Then
            MsgBox("事务处理失败", vbExclamation, "MAC Client")
            Selected()
        End If
    End Sub
    Friend Sub Selected()
        If Form.ListView4.SelectedItems.Count = 1 Then
            AID = CStr(Form.ListView4.SelectedItems.Item(0).Tag)
        End If
        QueryAffairsInfo()
    End Sub
    Private Sub QueryAffairsInfo()
        TextboxInfo.Text = Internet.GetAffairsInfo(Form.CurrentSocket, AID, Form.LoginCode)
        Textbox1.Text = Internet.GetAffairsSuggestion(Form.CurrentSocket, AID, Form.LoginCode)
    End Sub
End Class
